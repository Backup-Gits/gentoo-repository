...
if [[ ${PV} != *9999 ]]; then
	...
	EGIT_REPO_URI="https://git.haproxy.org/git/haproxy-1.8.git/"
	EGIT_BRANCH=master
fi
...
version_is_at_least 1.8.0 $PV && PATCHES=( "${FILESDIR}"/try-better-handle-empty-data.diff )
CONTRIBS=( halog iprange )
# ip6range is present in 1.6, but broken.
version_is_at_least 1.7.0 $PV && CONTRIBS+=( ip6range tcploop halog systemd iprange )
