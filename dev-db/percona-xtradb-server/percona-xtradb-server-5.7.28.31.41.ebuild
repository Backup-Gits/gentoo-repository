...
MY_RELEASE_NOTES_URI="https://www.percona.com/doc/percona-xtradb-cluster/${MY_MAJOR_PV}/release-notes/release-notes_index.html"

SRC_URI="https://www.percona.com/downloads/${MY_PN}-LATEST/${MY_PN}-${MY_PV}/source/tarball/${MY_PN}-${MY_PV}.tar.gz
...
HOMEPAGE="https://www.percona.com/software/mysql-database/percona-xtradb-cluster"
#DESCRIPTION="A fast, multi-threaded, multi-user SQL database server, cost-effective HA and scalability for MySQL"
LICENSE="GPL-2"
SLOT="0/18"
IUSE="cjk client-libs cracklib debug experimental jemalloc latin1 libressl numa pam +perl profiling
	selinux +server static static-libs systemtap tcmalloc test sphinx"
...
src_unpack() {
	...
	mv -f "${WORKDIR}/${MY_PN}-${MY_PV}" "${S}" || die
}
...
src_configure(){
	...
	append-cxxflags -felide-constructors -std=gnu++11
