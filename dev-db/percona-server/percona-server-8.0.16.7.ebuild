...
MY_RELEASE_NOTES_URI="https://www.percona.com/doc/percona-server/8.0/release-notes/release-notes_index.html"
SRC_URI="https://www.percona.com/downloads/${MY_PN}-${MY_MAJOR_PV}/${MY_PN}-${MY_PV}/source/tarball/${PN}-${MY_PV}.tar.gz
	mirror://sourceforge/boost/boost_1_69_0.tar.gz
"
S="${WORKDIR}/mysql"
...
IUSE="cjk cracklib debug jemalloc latin1 libressl numa +perl profiling
	router selinux tcmalloc test sphinx"
...
PATCHES=(
	...
	"${MY_PATCH_DIR}"/20018_all_percona-server-8.0.16-without-clientlibs-tools.patch
	"${MY_PATCH_DIR}"/20018_all_mysql-8.0.17-add-protobuf-3.8+-support.patch
	"${MY_PATCH_DIR}"/20018_percona-server-8.0.16-fix-libressl-support.patch
	"${FILESDIR}"/fts-punctation-word-8.patch
	"${FILESDIR}"/percona-server-8-buildfix.patch
)
...
src_configure(){
	...
	append-cxxflags -felide-constructors -fPIC -std=c++14 -ffp-contract=off

	# bug #283926, with GCC4.4, this is required to get correct behavior.
	append-flags -fno-strict-aliasing -fPIC -ffp-contract=off
	...
	mycmakeargs=(
		...
		-DWITH_CURL=system
		-DWITH_BOOST="${WORKDIR}/boost_1_69_0"
...
#with this option build will freeze random
#	if use jemalloc ; then
#		mycmakeargs+=( -DWITH_JEMALLOC=1 )
#	fi
